...
router.post("/AgregarSolicitud/:Id_Producto", helpers.OutStock, isLoggedIn, async(req, res) => {
    ...
    console.log(req.session.cart.CantTotal)
...
router.get("/SolicitudesGenerales", isLoggedIn, async(req, res) => {
    var solicitudes = await pool.query("SELECT * FROM `solicitudes` ORDER BY `solicitudes`.`F_Solicitud` DESC ");
    var nombre;
    //console.log(solicitudes);
    for (let v in solicitudes) {
...
router.get("/DetallesSolicitud/:Id_Solicitud", isLoggedIn, async(req, res) => {
    ...
    for (let v in solicitudes[0].PSolicitados) {
        const stock = await pool.query('SELECT * FROM productos WHERE Id_Producto= ?', [v]);
...
router.post("/DetallesSolicitud/:Id_Solicitud", isLoggedIn, async(req, res) => {
    //Incio Variables
    var Id_Solicitud = req.params.Id_Solicitud;
    var solicitudes = await pool.query("SELECT * FROM solicitudes WHERE Id_Solicitud=?", [Id_Solicitud]);
    ...
    await pool.query('UPDATE solicitudes set ? WHERE Id_Solicitud = ?', [Update, Id_Solicitud]);
    ...
    for (let v in prod) {
        ...
        if (newStock < 0) {
            req.flash("OutStock", "El producto " + stock[0].N_Producto + " No se pudo modificar\n Ya que se encuentra sin stock o stock insuficiente, verificar la informacion y realizar cambios respectivos\n");
        } else {
            await pool.query('UPDATE productos set Stock_Actual=? WHERE Id_Producto = ?', [newStock, v]);
        }
        i++;
    }
    i = 0;
...
router.get('/RechazarSolicitud/:Id_Solicitud', isLoggedIn, async(req, res) => {
    ...
    await pool.query('UPDATE `solicitudes` SET Estado=?,Id_Usuario_Entrega=?  WHERE Id_Solicitud = ?', [status, usuario, Id_Solicitud]);
