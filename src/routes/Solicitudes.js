...
router.get("/SolicitudesGenerales", isLoggedIn, async(req, res) => {
    ...
    res.render("Solicitudes/SolicitudesGenerales", { solicitudes, nombre, users });
});

router.post("/SolicitudesGenerales/Filtro", isLoggedIn, async(req, res) => {
    var users = await pool.query(
        "SELECT * FROM `users` WHERE Cargo='Funcionario' OR Cargo= 'Administrador'"
    );
    var sql = "SELECT * FROM solicitudes WHERE 1";
    if (req.body.Usuario != "Seleccione Funcionario") {
        sql += " AND Id_Usuario LIKE '%" + req.body.Usuario + "%'";
    }
    if (req.body.Estado != "Seleccione Estado") {
        sql += " AND Estado = '" + req.body.Estado + "'";
    }
...
router.get("/DetallesSolicitud/:Id_Solicitud", isLoggedIn, async(req, res) => {
    ...
    if (solicitudes[0].Estado == "Entregada") {
        ...
        let obj = JSON.parse(str);
        solicitudes[0].PrEntregados = obj;
    ...
    if (solicitudes[0].Estado == "Cancelada") {
        solicitudes[0].PrEntregados = false;
...
router.get("/DetallesMiSolicitud/:Id_Solicitud", isLoggedIn, async(req, res) => {
    //variables auxiliares
    var status = true;
    var status2 = false;
    ...
    //Asignación Icono según estado Solicitud
    if (solicitudes[0].Estado == "Entregada") {
        //Asignar Valores Productos Entregados
        let str = solicitudes[0].PrEntregados;
        let obj = JSON.parse(str);
        solicitudes[0].PrEntregados = obj;
        //Creacion variable bool para botones
